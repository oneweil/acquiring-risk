...
> 适用场景：一人全栈，按竖切主链路交付。登录已接 `sys_user`；用户/角色薄 RBAC 已落地，审计日志仍后置。

## 原则（单人必守）

1. **一次只竖切一条链路**：表 → 接口 → 页面，跑通再扩。
2. **不铺 16 个页面**：占位页保持占位，别先做 UI 全套。
3. **登录**：`sys_user` 校验，种子账号 `admin/admin123`（`SYS_ADMIN`）；操作人读 Session。
...
- [x] **3.1** 表：`risk_alert`（scope=order）+ `risk_alert_attachment`；后台列表/详情/调单上传/处置已实现（Seed 演示；无订单人审放行）
...
| 模块 | 何时做 |
|------|--------|
| 用户管理 / 角色权限 | **已实现**（薄 RBAC；无部门；审计日志仍后置） |
